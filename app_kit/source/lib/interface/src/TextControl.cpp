...
void BTextControl::Draw(BRect rect)
{
	...
	if (Label()){
		...
		SetHighColor(tint_color(ui_color(B_PANEL_BACKGROUND_COLOR), IsEnabled() ? B_DARKEN_MAX_TINT : B_DISABLED_LABEL_TINT));
	...
	rgb_color base = ui_color(B_PANEL_BACKGROUND_COLOR);
		...
		rect.InsetBy(-1,-1);
		SetHighColor(tint_color(base, IsEnabled() ? B_DARKEN_1_TINT : B_DARKEN_2_TINT));
		...
		SetHighColor(tint_color(base, IsEnabled() ? B_LIGHTEN_MAX_TINT : B_LIGHTEN_2_TINT));
...
BHandler *BTextControl::ResolveSpecifier(BMessage *msg, int32 index, BMessage *specifier, int32 form, const char *property)
{
	return NULL;
}
