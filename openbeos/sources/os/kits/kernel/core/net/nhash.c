...
/*
static void nhash_this(net_hash_index *hi, const void **key, ssize_t *klen,
                       void **val)
{
...
net_hash *nhash_make(void)
{
	net_hash *nn;
	 ...
	 nn = (net_hash *)kmalloc(sizeof(net_hash));
	...
	nn->array = (net_hash_entry **)kmalloc(sizeof(net_hash_entry) * (nn->max + 1));
...
static net_hash_index *nhash_next(net_hash_index *hi)
{
	hi->this = hi->next;
	while (!hi->this) {
...
static net_hash_index *nhash_first(net_hash *nh)
{
...
static void expand_array(net_hash *nh)
{
	...
	new_array = (net_hash_entry **)kmalloc(sizeof(net_hash_entry) * new_max);
